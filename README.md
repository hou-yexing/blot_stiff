...
其中 `hh`、`dh` 属于几何示意参数，不参与当前 `deltaSK` 拉伸柔度公式。双头螺柱模式下不计算螺栓头部 `deltaSK`；旋入端按盲孔拧入 `lM_in = 0.33d_in`，螺母端按螺母固定 `lM_nut = 0.4d_nut`，两端外螺纹等效长度均取 `lG = 0.5d`。双头螺柱夹紧长度必须满足 `l_free_in + sum(lRi) + l_free_nut = lK`，两端空余螺纹按对应端小径面积计入受拉柔度。
